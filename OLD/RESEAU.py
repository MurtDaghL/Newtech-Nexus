	...
	for i in range(E):
		...
		if i%I == 0:
			...
			for i in range(len(Apple.X2)):
				Openp.append(MMdenorm(Apple.X2[i][0],min(Open),max(Open)))
				Highp.append(MMdenorm(Apple.X2[i][1],min(High),max(High)))
				Lowp.append(MMdenorm(Apple.X2[i][2],min(Low),max(Low)))
				Closep.append(MMdenorm(Apple.X2[i][3],min(Close),max(Close)))
				Volumep.append(MMdenorm(Apple.X2[i][4],min(Volume),max(Volume)))
...
for i in range(len(z)):
	Openp.append(MMdenorm(z[i][0],min(Open),max(Open)))
	Highp.append(MMdenorm(z[i][1],min(High),max(High)))
	Lowp.append(MMdenorm(z[i][2],min(Low),max(Low)))
	Closep.append(MMdenorm(z[i][3],min(Close),max(Close)))
	Volumep.append(MMdenorm(z[i][4],min(Volume),max(Volume)))
# Raffraichissement des graphs:
Openg.refresh(1,x=range(len(Openp)),y=Openp)
Highg.refresh(1,x=range(len(Highp)),y=Highp)
Lowg.refresh(1,x=range(len(Lowp)),y=Lowp)
